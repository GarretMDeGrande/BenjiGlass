        ...
        <div class="cart-item">
          ...
          <div class="cart-item-info">
            ...
            <select class="cart-item-shipping" data-id="${item.id}">
              <option value="standard">Standard ($${item.standard.toFixed(2)})</option>
             <option value="express">Priority($${item.express.toFixed(2)})</option>
            </select>
          </div>
...
function calculateCartTotlal() {
  ...
  const totalShipping = cart.reduce((sum, item) => sum + (item.selectedShipping === 'standard' ? item.standard : item.express) * item.quantity, 0);
